        ...
        for (int i = 0; i < n; i++) {
            ...
            while (k < ni) {
                ...
                for (int r = 0; r < run_len; r += TILE_COLS) {
                    ...
                    TLOAD(tile_yj, GlobalTensor1D(&sy[j0], 1));
                    TLOAD(tile_zj, GlobalTensor1D(&sz[j0], 1));

                    /* TFILL: Broadcast xi/yi/zi */
                    TFILL(tile_xi, xi);
                    TFILL(tile_yi, yi);
                    TFILL(tile_zi, zi);
                    ...
                    /* TPBC: 周期边界条件 */
                    TPBC(tile_dx, box[0], inv_bx);
                    TPBC(tile_dy, box[1], inv_by);
                    ...
                    TLJ_FORCE(tile_fx, tile_fy, tile_fz,
                              tile_dx, tile_dy, tile_dz, tile_rsq,
                              lj_params,
