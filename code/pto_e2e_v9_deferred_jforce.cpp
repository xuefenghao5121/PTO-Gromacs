/* pto_e2e_v9_deferred_jforce.cpp
 * PTO-ISA v9: 延迟 j 力写回
 * 
 * 优化思路：
 * - v8 每个 tile 都写回 j 力（6 次 load/store）
 * - v9 把 j 力累积到临时数组，run 结束后一次性写回
 * - 减少内存访问次数
 */
...
/* TNONBONDED_LJ_NO_JFORCE: 只计算力，不写回 j 力 */
inline __attribute__((always_inline)) void TNONBONDED_LJ_NO_JFORCE(
    const float *sx, const float *sy, const float *sz,
    int j0, int tile_n,
    const NonBondedParams &p,
    float &fix, float &fiy, float &fiz,
    float *fx_out, float *fy_out, float *fz_out) {  /* 输出 fx/fy/fz 到临时数组 */
    ...
    /* 输出到临时数组 */
    svst1_f32(pg, fx_out, fx);
    svst1_f32(pg, fy_out, fy);
    svst1_f32(pg, fz_out, fz);
}
...
static double run_step(const float *sx, const float *sy, const float *sz,
    ...
    #pragma omp parallel
    {
        ...
        /* 临时数组存储 tile 的 fx/fy/fz */
        float fx_tmp[TILE_COLS], fy_tmp[TILE_COLS], fz_tmp[TILE_COLS];
        ...
        for (int i = 0; i < n; i++) {
            ...
            while (k < ni) {
                ...
                for (int r = 0; r < run_len; r += TILE_COLS) {
                    ...
                    TNONBONDED_LJ_NO_JFORCE(sx, sy, sz, j0, tile_n, p,
                                            fix, fiy, fiz,
                                            fx_tmp, fy_tmp, fz_tmp);
...
int main(int argc, char *argv[]) {
    ...
    printf("================================================================\n");
    printf("  PTO-GROMACS v9 — Deferred J-Force Writeback\n");
    printf("================================================================\n");
