                                  ...
                                  unsigned dstValidCol)
{
    ...
    static_assert(
        ...
        "Fix: TCOLARGMIN input data type must be f16/u16/f32/u32");
    ...
    static_assert(std::is_same_v<typename TileDataIn::DType, typename TileDataTmp::DType>,
                  "Fix: TCOLARGMIN input type must be consistent with the tmp type");
                                      ...
                                      unsigned srcValidCol)
{
    ...
    if (remainAfterLoop > 0) {
        ...
        copy_ubuf_to_ubuf(tmpPtr + tmpGapEles, srcPtr + numLoop * elemPerRpt, 0, 1, 8, 0, 0); // min elements
        for (uint16_t i = 1; i < srcValidRow; i++) {
...
PTO_INTERNAL void TCOLARGMIN_IMPL(TileDataOut &dst, TileDataIn &src, TileDataTmp &tmp)
{
    ...
    TColArgMinCheck<TileDataOut, TileDataIn, TileDataTmp>(srcValidRow, srcValidCol, dstValidRow, dstValidCol);

    if constexpr (sizeof(typename TileDataIn::DType) == 2) {
        TColArgMin16<TileDataOut, TileDataIn, TileDataTmp>(dst.data(), src.data(), tmp.data(), srcValidRow,
                                                           srcValidCol);
    } else if constexpr (sizeof(typename TileDataIn::DType) == 4) {
        TColArgMin32<TileDataOut, TileDataIn, TileDataTmp>(dst.data(), src.data(), tmp.data(), srcValidRow,
                                                           srcValidCol);
    }
}
} // namespace pto
