...
// ============================================================================
// Official MC2 tiling layout used by CANN 9.0 HCCL tiling APIs
// ============================================================================
...
private:
    ...
    bool AllocCommResource(int rankId, HcclComm commHandle, aclrtStream hcclStream, const char *group, void *&ctxPtr)
    {
        ...
        if (tilingRet != HCCL_SUCCESS) {
            std::cerr << "[ERROR] Rank " << rankId << ": official Mc2CcTilingConfig build failed: " << tilingRet
                      << std::endl;
            return false;
        }
...
static bool VerifyOutput(const uint16_t *output_fp16, const float *golden)
{
    ...
    constexpr size_t MAX_ERR_PRINT = 32;
                            ...
                            const std::vector<double> &pipe_comm_us)
{
    ...
    std::cout << "\n================================================================" << std::endl;
    std::cout << (is_ok ? "[SUCCESS]" : "[FAILED]") << " GEMM AllReduce A5 FP16 (HCCL)" << std::endl;
                               ...
                               size_t a_bytes, const uint16_t *b_data, size_t b_bytes, aclrtStream commStream)
{
    ...
    size_t winOffset = 0;
    // Reserve a small guard region so the live RS/AG buffer does not start at
    // the very beginning of the HCCL window.
    buf.reduced_output_head_pad = WindowAlloc(windowBase, winOffset, buf.reducedOutputHeadPadSize);
                                    ...
                                    const HcclRootInfo *rootInfo)
{
    ...
    if (!hctx.Init(rank_id, n_ranks, rootInfo, commStream)) {
...
{
    std::cout << "\n================================================================" << std::endl;
    std::cout << "  GEMM AllReduce A5 FP16 (ReduceScatter + AllGather) — HCCL backend" << std::endl;
    std::cout << "  M=" << G_ORIG_M << " K=" << G_K << " N=" << G_ORIG_N;
    if (G_M != G_ORIG_M || G_N != G_ORIG_N)
