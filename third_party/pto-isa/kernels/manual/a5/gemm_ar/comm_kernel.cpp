...
//
// Only comm_core_idx==0 performs cross-rank TNOTIFY/TWAIT signaling.
...
//   [MAX_RANKS+1]                   Intra-rank block arrival counter (for RS completion)
                                 ...
                                 int nranks, int comm_core_idx, int num_comm_blocks, int32_t expected = 1)
{
    ...
    __gm__ int32_t *arrival = signal_base + SIGNAL_INTRA_RANK_COUNTER + phase;
    {
    ...
    if (comm_core_idx == 0) {
        {
            pto::comm::Signal arrSig(arrival);
            pto::comm::TWAIT(arrSig, (int32_t)num_comm_blocks, pto::comm::WaitCmp::GE);
        }
...
AICORE inline void ReduceScatterPhase(__gm__ half *gemm_output, __gm__ half *reduced_output,
                                      ...
                                      int my_rank, int nranks, int num_compute_blocks, int comm_core_idx,
                                      int num_comm_blocks)
{
                                  ...
                                  int nranks, int comm_core_idx, int num_comm_blocks)
{
    ...
    int row_start = comm_core_idx * rows_per_block;
    int row_end = (comm_core_idx + 1) * rows_per_block;
                                   ...
                                   int nranks, int num_compute_blocks, int comm_core_idx, int num_comm_blocks)
{
    ...
    ReduceScatterPhase(gemm_output, reduced_output, queue_set, hcclCtx, my_rank, nranks, num_compute_blocks,
                       comm_core_idx, num_comm_blocks);

    DeviceBarrier(hcclCtx, signal_matrix, 0, my_rank, nranks, comm_core_idx, num_comm_blocks);

    AllGatherPhase(reduced_output, hcclCtx, my_rank, nranks, comm_core_idx, num_comm_blocks);
}
