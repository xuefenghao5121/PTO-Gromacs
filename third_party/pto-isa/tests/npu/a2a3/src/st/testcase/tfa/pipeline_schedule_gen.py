...
def schedule_pipeline(
    ...
    for t in range(num_tiles):
        ...
        qk_done, cube_comp_free = schedule_stage(
            t, qk, 0, io_load_overhead, io_store_overhead, cube_comp_free, io_load_free, io_store_free, pending_comp, tasks
        )

        # P scheduling (depends on qk(t) done; vector load/comp/store)
        p_start_dep = qk_done
        p_done_t, vec_comp_free = schedule_stage(
            t, p, p_start_dep, io_load_overhead, io_store_overhead, vec_comp_free, io_load_free, io_store_free, pending_comp, tasks
        )
        p_done[t] = p_done_t
        ...
        if len(pv_queue) >= max(1, preload + 1):
            ...
            pv_done, cube_comp_free = schedule_stage(
                tile_idx, pv, pv_start_dep, io_load_overhead, io_store_overhead, cube_comp_free, io_load_free, io_store_free, pending_comp, tasks
            )

            gu_start_dep = pv_done
            _, vec_comp_free = schedule_stage(
                tile_idx, gu, gu_start_dep, io_load_overhead, io_store_overhead, vec_comp_free, io_load_free, io_store_free, pending_comp, tasks
            )
