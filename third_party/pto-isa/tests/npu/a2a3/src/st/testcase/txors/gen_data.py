...
def gen_golden_data(case_name, param):
    ...
    input2 = np.random.randint(dtype_info.min, dtype_info.max, size=[1, 1]).astype(dtype)
...
def generate_case_name(param):
    ...
    return f"TXORSTest.case_{dtype_str}_{param.dst_tile_row}x{param.dst_tile_col}_\
...
if __name__ == "__main__":
    ...
    case_params_list = [
        TXorSParams(np.int16, 64, 64, 64, 64, 64, 64),
        TXorSParams(np.int16, 32, 128, 32, 128, 32, 128),
        TXorSParams(np.int16, 32, 112, 32, 128, 32, 111),
        TXorSParams(np.uint16, 64, 64, 64, 64, 64, 64),
        TXorSParams(np.uint16, 32, 128, 32, 128, 32, 128),
        TXorSParams(np.uint16, 32, 112, 32, 128, 32, 111),
    ]
