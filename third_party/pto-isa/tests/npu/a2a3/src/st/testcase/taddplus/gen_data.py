...
def gen_golden_data_taddplus(case_name, param):
    dtype = param.dtype
    ...
    golden = input1[0:dst_tile_row, 0:dst_tile_col] + input2[0:dst_tile_row, 0:dst_tile_col]
...
def generate_case_name(param):
    ...
    return f"TADDPLUSTest.case_{dtype_str}_{param.dst_tile_row}x{param.dst_tile_col}_{param.src0_tile_row}x{param.src0_tile_col}_{param.src1_tile_row}x{param.src1_tile_col}_{param.valid_row}x{param.valid_col}"
...
if __name__ == "__main__":
    ...
    case_params_list = [
        TAddPlusParams(np.float16, 16, 64, 16, 128, 16, 128, 16, 64),
        TAddPlusParams(np.float32, 16, 32, 16, 64, 16, 32, 16, 32),
        TAddPlusParams(np.int16, 32, 128, 32, 128, 32, 256, 32, 128),
        TAddPlusParams(np.int32, 16, 32, 16, 64, 16, 32, 16, 32),
        TAddPlusParams(np.float16, 16, 64, 16, 128, 16, 128, 16, 63),
        TAddPlusParams(np.float32, 16, 32, 16, 64, 16, 32, 16, 31),
        TAddPlusParams(np.int16, 32, 128, 32, 128, 32, 256, 32, 127),
        TAddPlusParams(np.int32, 16, 32, 16, 64, 16, 32, 16, 31),
    ]
    ...
    for param in case_params_list:
        ...
        os.chdir(case_name)
        gen_golden_data_taddplus(case_name, param)
