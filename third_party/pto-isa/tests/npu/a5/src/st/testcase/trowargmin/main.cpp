...
protected:
    ...
    void Launch()
    {
        ...
        if constexpr (isHalf) {
            LaunchTRowArgMinHalf<TIdx, dstValTileH, dstValTileW, dstTileH, dstTileW, srcTileH, srcTileW, vRows, vCols>(
                (TVal *)this->dstValDevice, (TIdx *)this->dstDevice, (TVal *)this->srcDevice, this->stream);
        } else {
            LaunchTRowArgMin<TIdx, TVal, dstValTileH, dstValTileW, dstTileH, dstTileW, srcTileH, srcTileW, vRows,
