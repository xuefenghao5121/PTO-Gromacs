                                                   ...
                                                   __gm__ OutT *bias)
{
    ...
    if constexpr (DAV_VEC) {
        // In TILE_NO_SPLIT mode only AIV0 receives data from the FIFO.
        // AIV1 skips the loop entirely and just participates in pipe_barrier.
        if (get_subblockid() == 0) {
            BiasTile biasTile;
            OutTile outTile;
            ...
            for (int m_tile = 0; m_tile < NUM_M_TILES; m_tile++) {
                ...
                // Signal Cube that the FIFO slot is free.
