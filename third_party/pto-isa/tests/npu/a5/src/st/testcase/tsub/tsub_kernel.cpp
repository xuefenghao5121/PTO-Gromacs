...
__global__ AICORE void runTSub(__gm__ T __out__ *out, __gm__ T __in__ *src0, __gm__ T __in__ *src1)
{
    ...
    Event<Op::TLOAD, Op::TSUB> event0;
    Event<Op::TSUB, Op::TSTORE_VEC> event1;

    TLOAD(src0Tile, src0Global);
    event0 = TLOAD(src1Tile, src1Global);
    event1 = TSUB(dstTile, src0Tile, src1Tile, event0);
...
__global__ AICORE void runTSub(__gm__ T __out__ *out, __gm__ T __in__ *src0, __gm__ T __in__ *src1)
{
    ...
    TLOAD(src0Tile, src0Global);
    event0 = TLOAD(src1Tile, src1Global);
    event1 = TSUB<TileDataDst, TileDataSrc0, TileDataSrc1>(dstTile, src0Tile, src1Tile, event0);
    TSTORE(dstGlobal, dstTile, event1);
    out = dstGlobal.data();
}
...
void LaunchTSub(T *out, T *src0, T *src1, void *stream)
{
    if constexpr (sameTile) {
        ...
        runTSub<T, dstTileH, dstTileW, src0TileH, src0TileW, src1TileH, src1TileW, vRows, vCols>
