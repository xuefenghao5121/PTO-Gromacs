...
extern "C" __global__ AICORE void launchTSCATTERCase8(__gm__ bfloat16_t *out, __gm__ bfloat16_t *src,
                                                      __gm__ int16_t *indexes)
{
...
void launchTScatterTestCase(void *out, void *src, void *indexes, aclrtStream stream)
{
    switch (caseId) {
        ...
        case 8: {
            launchTSCATTERCase8<<<1, nullptr, stream>>>(reinterpret_cast<bfloat16_t *>(out),
                                                        reinterpret_cast<bfloat16_t *>(src), (int16_t *)indexes);
            break;
        }
        default: {
        }
