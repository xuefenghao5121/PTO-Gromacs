...
void TPushPopMatmulAddNoSplitTestFunc(uint32_t M, uint32_t K, uint32_t N)
{
    ...
    LaunchTPushPopMatmulAddNoSplit<key>(dstDevice, srcADevice, srcBDevice, biasDevice, stream);
...
// Single M-tile, half-precision input, float accumulation.
TEST_F(TPushPopCVNoSplitTest, case1_half_single_tile)
{
    TPushPopMatmulAddNoSplitTestFunc<aclFloat16, float, 1>(16, 32, 32);
}

// Two M-tiles: exercises FIFO ring-buffer wrap-around with TILE_NO_SPLIT.
TEST_F(TPushPopCVNoSplitTest, case2_half_two_tiles)
{
    TPushPopMatmulAddNoSplitTestFunc<aclFloat16, float, 2>(32, 32, 32);
}

// Single M-tile with float input (no type conversion path).
