...
__global__ AICORE void runTShl(__gm__ T *out, __gm__ T *src0, __gm__ T *src1)
{
    ...
    Event<Op::TLOAD, Op::TSHL> event0;
    Event<Op::TSHL, Op::TSTORE_VEC> event1;

    TLOAD(src0Tile, src0Global);
    event0 = TLOAD(src1Tile, src1Global);
    event1 = TSHL(dstTile, src0Tile, src1Tile, event0);
...
{
    runTShl<T, kTRows_, kTCols_, vRows, vCols><<<1, nullptr, stream>>>(out, src0, src1);
}
...
template void LaunchTShl<int8_t, 32, 32, 32, 32>(int8_t *out, int8_t *src0, int8_t *src1, void *stream);
template void LaunchTShl<int16_t, 16, 16, 16, 16>(int16_t *out, int16_t *src0, int16_t *src1, void *stream);
