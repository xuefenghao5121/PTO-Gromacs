...
def gen_golden_data_taddsc(case_name, param):
    dtype = param.dtype
    ...
    golden[:row_valid, :col_valid] = NumExt.astype(input1 + scalar + input2, dtype)[:row_valid, :col_valid]
...
def generate_case_name(param):
    ...
    name = f"TADDSCTest.case_{dtype_str}" 
...
if __name__ == "__main__":
    ...
    case_params_list = [
        TAddscParams(np.float32, 64, 64, 64, 64, 64, 64),
        TAddscParams(np.int32, 64, 64, 64, 64, 64, 64),
        TAddscParams(np.int16, 64, 64, 64, 64, 64, 64),
        TAddscParams(np.float16, 16, 256, 16, 256, 16, 256)
    ]
    if os.getenv("PTO_CPU_SIM_ENABLE_BF16") == "1":
        case_params_list.append(TAddscParams(NumExt.bf16, 16, 256, 16, 256, 16, 256))
    ...
    for i, param in enumerate(case_params_list):
        ...
        os.chdir(case_name)
        gen_golden_data_taddsc(case_name, param)
