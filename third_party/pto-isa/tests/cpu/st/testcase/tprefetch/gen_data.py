...
def gen_golden_data_tprefetch(case_name, param):
    dtype = param.dtype
...
if __name__ == "__main__":
    ...
    case_params_list = [
        TPrefetchParams(np.float32, 64, 64, 64, 64, 64, 64),
        TPrefetchParams(np.int32, 64, 64, 64, 64, 64, 64),
        TPrefetchParams(np.int16, 64, 64, 64, 64, 64, 64),
        TPrefetchParams(np.float16, 16, 256, 16, 256, 16, 256),
    ]
    if os.getenv("PTO_CPU_SIM_ENABLE_BF16") == "1":
        case_params_list.append(TPrefetchParams(NumExt.bf16, 16, 256, 16, 256, 16, 256))
    ...
    for i, param in enumerate(case_params_list):
        ...
        os.chdir(case_name)
        gen_golden_data_tprefetch(case_name, param)
