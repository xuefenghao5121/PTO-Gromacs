...
`pto.vsubrelu` 属于[SFU 与 DSA 操作](../../sfu-and-dsa-ops_zh.md)指令集。

## 概述

把逐 lane 减法和 ReLU 融合成一条指令。
...
它把“减法 + ReLU”压成单条硬件操作，减少中间寄存器结果落地，同时保留融合后的数值与吞吐语义。
...
| 结果 | 类型 | 说明 |
|------|------|------|
| `%result` | `!pto.vreg<NxT>` | 融合 sub-then-ReLU 结果 |
...
- 约束部分列出的额外非法情形，同样属于 `pto.vsubrelu` 的契约。
...
对于 `pto.vsubrelu`，这些公开来源说明了指令语义、操作数合法性和流水线位置，但**没有**发布数字时延或稳态吞吐。
...
如果软件调度或性能建模依赖 `pto.vsubrelu` 的确切成本，必须在具体 backend 上实测，而不能从当前公开手册里推导出一个并未公布的常数。

## 相关页面

- 指令集总览：[SFU 与 DSA 操作](../../sfu-and-dsa-ops_zh.md)
- 上一条指令：[pto.vaddrelu](./vaddrelu_zh.md)
- 下一条指令：[pto.vaxpy](./vaxpy_zh.md)
