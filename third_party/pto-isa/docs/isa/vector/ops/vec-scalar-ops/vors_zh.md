...
`pto.vors` 属于[向量-标量操作](../../vec-scalar-ops_zh.md)指令集。

## 概述

`pto.vors` 对一个向量寄存器和一个广播标量做逐 lane 按位或。
...
| 结果 | 类型 | 说明 |
|------|------|------|
| `%result` | `!pto.vreg<NxT>` | 活跃 lane 上得到逐 lane 按位或结果 |
...
- 约束部分列出的额外非法情形，同样属于 `pto.vors` 的契约。
...
- 上一条指令：[pto.vands](./vands_zh.md)
- 下一条指令：[pto.vxors](./vxors_zh.md)
