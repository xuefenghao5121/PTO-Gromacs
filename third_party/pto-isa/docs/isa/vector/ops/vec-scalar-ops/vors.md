...
`pto.vors` is part of the [Vector-Scalar Instructions](../../vec-scalar-ops.md) instruction set.

## Summary

Lane-wise bitwise OR of a vector register and a broadcast scalar.
...
```text
vors %dst, %src, %scalar, %mask : !pto.vreg<NxT>, T
```
...
| --- | --- | --- |
| %result | `!pto.vreg<NxT>` | Lane-wise bitwise OR result on the active lanes |
...
```mlir
%result = pto.vors %values, %bitmask, %mask : !pto.vreg<64xi32>, i32, !pto.mask -> !pto.vreg<64xi32>
```
