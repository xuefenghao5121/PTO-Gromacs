...
`pto.vshls` 属于[向量-标量操作](../../vec-scalar-ops_zh.md)指令集。

## 概述

`pto.vshls` 对一个向量寄存器做逐 lane 左移，位移量来自同一个广播标量。
...
| 结果 | 类型 | 说明 |
|------|------|------|
| `%result` | `!pto.vreg<NxT>` | 活跃 lane 上得到逐 lane 左移结果 |
...
- 约束部分列出的额外非法情形，同样属于 `pto.vshls` 的契约。
...
对于 `pto.vshls`，这些公开来源说明了指令语义、操作数合法性和流水线位置，但**没有**发布数字时延或稳态吞吐。
...
如果软件调度或性能建模依赖 `pto.vshls` 的确切成本，必须在具体 backend 上实测，而不能从当前公开手册里推导出一个并未公布的常数。

## 相关页面

- 指令集总览：[向量-标量操作](../../vec-scalar-ops_zh.md)
- 上一条指令：[pto.vxors](./vxors_zh.md)
- 下一条指令：[pto.vshrs](./vshrs_zh.md)
