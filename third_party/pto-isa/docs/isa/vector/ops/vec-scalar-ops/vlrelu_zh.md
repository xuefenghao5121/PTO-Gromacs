...
`pto.vlrelu` 属于[向量-标量操作](../../vec-scalar-ops_zh.md)指令集。

## 概述

`pto.vlrelu` 是带广播斜率标量的逐 lane Leaky ReLU。
...
标量 `%scalar` 为每个活跃 lane 提供负半轴斜率。非活跃 lane 不参与计算。
...
| 结果 | 类型 | 说明 |
|------|------|------|
| `%result` | `!pto.vreg<NxT>` | 活跃 lane 上得到逐 lane Leaky ReLU 结果 |
...
- 约束部分列出的额外非法情形，同样属于 `pto.vlrelu` 的契约。
...
- 上一条指令：[pto.vshrs](./vshrs_zh.md)
- 下一条指令：[pto.vaddcs](./vaddcs_zh.md)
