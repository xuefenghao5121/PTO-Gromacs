...
`pto.vmins` 属于[向量-标量操作](../../vec-scalar-ops_zh.md)指令集。

## 概述

`pto.vmins` 取一个向量寄存器和一个广播标量的逐 lane 最小值。

## 机制

对每个活跃 lane `i`，执行 `dst[i] = min(src[i], scalar)`。标量会广播到所有活跃 lane。非活跃 lane 不参与比较。
...
| 结果 | 类型 | 说明 |
|------|------|------|
| `%result` | `!pto.vreg<NxT>` | 活跃 lane 上得到逐 lane 最小值 |
...
- 约束部分列出的额外非法情形，同样属于 `pto.vmins` 的契约。
...
- 上一条指令：[pto.vmaxs](./vmaxs_zh.md)
- 下一条指令：[pto.vands](./vands_zh.md)
