...
`pto.vaddcs` 属于[向量-标量操作](../../vec-scalar-ops_zh.md)指令集。

## 概述

`pto.vaddcs` 是带显式 carry-in / carry-out 掩码的逐 lane 加法。
...
```text
sum = lhs[i] + rhs[i] + carry_in[i]
result[i] = low_bits(sum)
carry[i]  = carry_out(sum)
```

在 PTO 可见表面里，这条 carry 链是逐 lane 独立的：每个 lane 消费一个输入进位 bit，并产出一个输出进位 bit。
...
这条指令除了产生目标值和 carry 掩码，没有其他架构副作用。

## 约束

- carry 链形式只对整数元素类型定义。
- `%lhs`、`%rhs` 与 `%result` 必须具有相同的向量宽度 `N` 和相同的元素类型 `T`。
- `%carry_in`、`%carry` 与 `%mask` 的宽度都必须为 `N`。

## 异常与非法情形

- verifier 会拒绝非法的操作数形状、不支持的元素类型以及不合法的属性组合。
- 约束部分列出的额外非法情形，同样属于 `pto.vaddcs` 的契约。

## 目标 Profile 限制

- 除非某个目标 profile 明确扩大合法域，否则应把这条形式视为无符号整数 carry-chain。
...
对于 `pto.vaddcs`，这些公开来源说明了指令语义、操作数合法性和流水线位置，但**没有**发布数字时延或稳态吞吐。
...
如果软件调度或性能建模依赖 `pto.vaddcs` 的确切成本，必须在具体 backend 上实测，而不能从当前公开手册里推导出一个并未公布的常数。

## 相关页面

- 指令集总览：[向量-标量操作](../../vec-scalar-ops_zh.md)
- 上一条指令：[pto.vlrelu](./vlrelu_zh.md)
- 下一条指令：[pto.vsubcs](./vsubcs_zh.md)
