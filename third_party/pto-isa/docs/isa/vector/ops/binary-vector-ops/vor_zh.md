...
`pto.vor` 属于[二元向量操作](../../binary-vector-ops_zh.md)指令集。

## 概述

`pto.vor` 在每个活跃 lane 上做按位或：
...
这条指令对两个源向量寄存器做逐 lane 按位或。对每个满足谓词的 lane `i`：

$$ \mathrm{dst}_i = \mathrm{lhs}_i \,|\, \mathrm{rhs}_i $$

非活跃 lane 保持目标原值。它是纯整数指令，不支持浮点元素类型。
...
| 结果 | 类型 | 说明 |
|------|------|------|
| `%result` | `!pto.vreg<NxT>` | 活跃 lane 上得到逐 lane 按位或；非活跃 lane 保持原值 |
...
- [二元向量操作](../../binary-vector-ops_zh.md)页面列出的额外非法情形，同样适用于 `pto.vor`。
...
- 上一条指令：[pto.vand](./vand_zh.md)
- 下一条指令：[pto.vxor](./vxor_zh.md)
