...
`pto.vand` is part of the [Binary Vector Instructions](../../binary-vector-ops.md) instruction set.

## Summary

Lane-wise bitwise AND: `dst[i] = lhs[i] & rhs[i]` for each active lane.

## Mechanism

Computes the lane-wise bitwise AND of two source vector registers. For each lane `i` where the predicate is true:
...
|--------|------|-------------|
| `%result` | `!pto.vreg<NxT>` | Lane-wise bitwise AND: `dst[i] = lhs[i] & rhs[i]` on active lanes; inactive lanes are unmodified |
