...
`pto.vshl` 属于[二元向量操作](../../binary-vector-ops_zh.md)指令集。

## 概述

`pto.vshl` 在每个活跃 lane 上做左移：

```text
dst[i] = lhs[i] << rhs[i]
```

这里的位移量不是统一标量，而是来自右侧向量寄存器的逐 lane 位移计数。

## 机制

这条指令用 `%rhs` 每个 lane 上的计数去左移 `%lhs` 对应 lane 上的值。对每个满足谓词的 lane `i`：

$$ \mathrm{dst}_i = \mathrm{lhs}_i \ll \mathrm{rhs}_i $$

`rhs[i]` 按无符号位移量解释。移出的比特会被丢弃。非活跃 lane 保持目标原值。
...
| 结果 | 类型 | 说明 |
|------|------|------|
| `%result` | `!pto.vreg<NxT>` | 活跃 lane 上得到逐 lane 左移结果；非活跃 lane 保持原值 |
...
- 三个寄存器的向量宽度必须一致。
- `%mask` 的宽度必须与 `N` 一致。
- 位移量最好保持在 `[0, bitwidth(T) - 1]` 范围内；超范围行为由目标平台定义，除非 verifier 进一步收窄。
- 非活跃 lane 保持目标原值。

## 异常与非法情形

- verifier 会拒绝非整数元素类型、类型不匹配、宽度不匹配或掩码宽度不匹配。
- [二元向量操作](../../binary-vector-ops_zh.md)页面列出的额外非法情形，同样适用于 `pto.vshl`。
...
%shifted = pto.vshl %data, %count, %active
    : (!pto.vreg<64xi32>, !pto.vreg<64xi32>, !pto.mask) -> !pto.vreg<64xi32>

%shifted2 = pto.vshl %data, %counts, %active
    : (!pto.vreg<64xi32>, !pto.vreg<64xi32>, !pto.mask) -> !pto.vreg<64xi32>
```
...
对于 `pto.vshl`，这些公开来源说明了指令语义、操作数合法性和流水线位置，但**没有**发布数字时延或稳态吞吐。
...
如果软件调度或性能建模依赖 `pto.vshl` 的确切成本，必须在具体 backend 上实测，而不能从当前公开手册里推导出一个并未公布的常数。

## 相关页面

- 指令集总览：[二元向量操作](../../binary-vector-ops_zh.md)
- 上一条指令：[pto.vxor](./vxor_zh.md)
- 下一条指令：[pto.vshr](./vshr_zh.md)
