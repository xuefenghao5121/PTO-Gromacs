...
`pto.vsunpack` 属于[数据重排](../../data-rearrangement_zh.md)指令集。
...
`pto.vsunpack` 从源向量里选中一半，再把每个窄元素做符号扩展，写成更宽的目标向量。
...
| 结果 | 类型 | 说明 |
|------|------|------|
| `%result` | `!pto.vreg<N/2xT_wide>` | 经过符号扩展的更宽结果向量 |
...
- 约束部分列出的额外非法情形，同样属于 `pto.vsunpack` 的契约。
...
当前仓内没有为 `vsunpack` 单列周期表。若代码对具体延迟敏感，应把它视为目标 profile 相关的扩宽重排路径。
...
- 上一条指令：[pto.vpack](./vpack_zh.md)
- 下一条指令：[pto.vzunpack](./vzunpack_zh.md)
