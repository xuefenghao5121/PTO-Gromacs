...
`pto.tmin` is part of the [Elementwise Tile Tile](../../elementwise-tile-tile.md) instruction set.
...
|--------|------|-------------|
| `%dst` | `!pto.tile<...>` | Destination tile; all `(i, j)` in its valid region contain `min(src0[i,j], src1[i,j])` after the operation |
...
`TMIN` compiles to CCE vector instructions via the `TBinOp.hpp` performance model. The throughput is identical to `TADD` (binary arithmetic):
