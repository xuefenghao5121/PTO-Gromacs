...
`pto.tmin` 属于[逐元素 Tile-Tile](../../elementwise-tile-tile_zh.md)指令集。
...
| 结果 | 类型 | 说明 |
| --- | --- | --- |
| `%dst` | `!pto.tile<...>` | `dst` valid region 内的每个元素都等于 `min(src0, src1)` |
...
当前手册未单列 `tmin` 的独立周期表，应视为目标 profile 相关。
...
- 上一条指令：[pto.tmul](./tmul_zh.md)
- 下一条指令：[pto.tmax](./tmax_zh.md)
