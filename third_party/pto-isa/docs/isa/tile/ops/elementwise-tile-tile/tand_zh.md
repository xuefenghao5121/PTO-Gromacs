...
`pto.tand` 属于[逐元素 Tile-Tile](../../elementwise-tile-tile_zh.md)指令集。
...
这条指令要求源和目标都按整数语义解释，适合做位图组合、掩码筛选和按位裁剪。
...
就语义而言，这条指令主要面向整数；表中的 FP 列只是沿用统一模型写法，不表示浮点 tile 是推荐用法。

### A5

当前手册未单列 `tand` 的独立周期表，应视为目标 profile 相关。
...
- 上一条指令：[pto.tabs](./tabs_zh.md)
- 下一条指令：[pto.tor](./tor_zh.md)
