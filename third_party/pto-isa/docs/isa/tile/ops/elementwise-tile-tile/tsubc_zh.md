...
`pto.tsubc` 属于[逐元素 Tile-Tile](../../elementwise-tile-tile_zh.md)指令集。

## 概述

三输入逐元素运算：`src0 - src1 + src2`。
...
它和 `taddc` 一样，强调的是三输入融合算术，而不是普通二元运算的简单延伸。
...
- `pto.tsubc` 在 CPU 仿真、A2/A3 和 A5 上保留一致的 PTO 可见语义，但具体支持子集仍取决于 profile。

## 性能

### A2A3

英文页当前把 `TSUBC` 归到与二元算术同类的估算口径：
...
当前手册未单列 `tsubc` 的独立周期表，应视为目标 profile 相关。
...
- 上一条指令：[pto.taddc](./taddc_zh.md)
- 下一条指令：[pto.tcvt](./tcvt_zh.md)
