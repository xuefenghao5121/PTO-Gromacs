...
`pto.tors` 属于[Tile-标量与立即数](../../tile-scalar-and-immediate_zh.md)指令集。
...
标量会广播到整个 valid region。它常用于逐元素置位、合并标志位以及对 tile 做统一位图叠加。
...
当前仓内没有为 `tors` 单列公开 cost bucket。若代码依赖具体延迟，应把它视为目标 profile 相关的 tile 按位逻辑路径。
...
- 上一条指令：[pto.tands](./tands_zh.md)
- 下一条指令：[pto.tshls](./tshls_zh.md)
