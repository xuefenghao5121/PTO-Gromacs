...
`pto.tadds` 属于[Tile-标量与立即数](../../tile-scalar-and-immediate_zh.md)指令集。
...
`pto.tadds` 作用在 tile payload 上，而不是标量控制状态。对目标 tile 的有效区域内每个元素 `(i, j)`：

$$ \mathrm{dst}_{i,j} = \mathrm{src}_{i,j} + \mathrm{scalar} $$

其中 `scalar` 逻辑上广播到整个 valid region。合法性由 tile 形状、布局、valid-region 以及目标 profile 共同约束。
...
除产生目标 tile 外，没有额外架构副作用，不会隐式为无关访存建立栅栏。
...
- tile 位置必须是向量 tile（`TileType::Vec`）
- 静态 valid 边界必须满足 `ValidRow <= Rows`、`ValidCol <= Cols`
...
仓内 `include/pto/costmodel/pto_isa_costmodel.hpp` 直接给出 `TADDS -> vadds` 对应桶：

| 指标 | INT16 / INT32 | FP16 / FP32 |
| --- | --- | --- |
| 启动时延 | 14 (`A2A3_STARTUP_BINARY`) | 14 (`A2A3_STARTUP_BINARY`) |
| 完成时延 | 17 (`A2A3_COMPL_INT_BINOP`) | 19 (`A2A3_COMPL_FP_BINOP`) |
| 每次 repeat 吞吐 | 1 (`A2A3_RPT_1`) | 1 (`A2A3_RPT_1`) |
| 流水间隔 | 18 (`A2A3_INTERVAL`) | 18 (`A2A3_INTERVAL`) |

### A5

当前手册未单列 `tadds` 的独立周期条目，应视为目标 profile 相关。
...
- 上一条指令：[pto.tmins](./tmins_zh.md)
- 下一条指令：[pto.tsubs](./tsubs_zh.md)
