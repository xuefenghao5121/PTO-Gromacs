...
`pto.tcolexpandexpdif` is part of the [Reduce And Expand](../../reduce-and-expand.md) instruction set.
...
void example_auto() {
  ...
  DstT dst;
  ColVecT src1;
  // Col-expand-expdif: each column of dst = exp(src0.col - src1.col_scalar)
  TCOLEXPANDEXPDIF(dst, src0, src1);
}
```
...
void example_manual() {
  ...
  TASSIGN(dst, 0x2000);
  TASSIGN(src1, 0x3000);
  // Col-expand-expdif in manual mode
  TCOLEXPANDEXPDIF(dst, src0, src1);
}
```
