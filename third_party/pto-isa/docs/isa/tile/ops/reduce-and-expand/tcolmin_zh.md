...
`pto.tcolmin` 属于[归约与扩展](../../reduce-and-expand_zh.md)指令集。
...
当前仓内没有把 `tcolmin` 单列成公开 cost table。它应视为列归约路径，而不是普通逐元素算术。
