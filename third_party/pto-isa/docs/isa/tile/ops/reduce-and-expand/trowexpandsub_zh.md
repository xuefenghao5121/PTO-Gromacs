...
把“每行一个标量”的向量广播到整行，再从 `src0` 中逐元素减去它。
...
当前仓内没有把 `trowexpandsub` 单列成公开 cost table，应视为目标 profile 相关的广播组合路径。
...
- 上一条指令：[pto.trowexpandmul](./trowexpandmul_zh.md)
- 下一条指令：[pto.trowexpandadd](./trowexpandadd_zh.md)
