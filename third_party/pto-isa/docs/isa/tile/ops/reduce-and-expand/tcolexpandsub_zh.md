...
`pto.tcolexpandsub` 属于[归约与扩展](../../reduce-and-expand_zh.md)指令集。

## 概述

把“每列一个标量”的向量广播到整列，再从 `src0` 中逐元素减去它。
...
当前仓内没有把 `tcolexpandsub` 单列成公开 cost table，应视为目标 profile 相关的列广播组合路径。
